...
public class NanoHTTPD
{
    ...
    private JSONObject createJSONRequest(String requestId, String uri, String method, Properties header, String rawInput) throws JSONException {
        JSONObject jsonRequest = new JSONObject();
        jsonRequest.put("headers", header);
        jsonRequest.put("requestId", requestId);
        jsonRequest.put("method", method);
        jsonRequest.put("path", uri);
        jsonRequest.put("rawInput", rawInput);
	...
	public Response serve( String uri, String method, Properties header, Properties parms, String rawInput, Properties files )
	{
...
*/
		...
		return serveJavascript( uri, method, header, rawInput, myRootDir, true );
...
/**
	 ...
	 */
	public NanoHTTPD(InetSocketAddress localAddr, AndroidFile wwwroot, Dserver s) throws IOException
	{
		Log.d(this.getClass().getName(), "danielLog c1: entrou NanoHTTPD()");
		this.DServer = s;

		Log.d(this.DServer.getClass().getName(), "danielLog c1: depois de this.corhttpd = s ");
		...
		Log.d(this.getClass().getName(), "danielLog  antes myThread.setDaemon");
		myThread.setDaemon( true );
		Log.d(this.getClass().getName(), "danielLog  depois myThread.setDaemon");
		this.myThread.setName("NanoHttpd Main Listener");
		myThread.start();
		Log.d(this.getClass().getName(), "danielLog  depois myThread.start");
	}
	 ...
	 * Throws an IOException if the socket is already in use
	 */
	public NanoHTTPD( int port, AndroidFile wwwroot, Dserver s) throws IOException
	{	
		Log.d(this.getClass().getName(), "danielLog c2: entrou NanoHTTPD()");
		this.DServer = s;
		Log.d(this.DServer.getClass().getName(), "danielLog  c2: depois de this.corhttpd = s ");
			...
			Thread t = new Thread( new Runnable()
			{
				public void run()
				{
					try
					{
						...
						// Here there will be added the Raw Input body of a POST REQUEST, if there is any:
						String postLine = "";
	...
	public Response serveJavascript( String uri, String method, Properties header, String rawInput, AndroidFile homeDir,
			boolean allowDirectoryListing )
	{
        ...
        try {
            ...
            //Aqui cria o JSON com informa��o do request
        	
            pluginResult = new PluginResult(
                    PluginResult.Status.OK, this.createJSONRequest(requestUUID, uri, method, header, rawInput));
        } catch (JSONException e) {
            e.printStackTrace();
        }
        	
        pluginResult.setKeepCallback(true);
        this.DServer.onRequestCallbackContext.sendPluginResult(pluginResult);

        while (!this.DServer.responses.containsKey(requestUUID)) {
            try {
                Thread.sleep(1);
            } catch (InterruptedException e) {
                e.printStackTrace();
        ...
        //ACHO QUE � AQUI QUE EST� A CRIAR UMA NOVA RESPONSE
        
        JSONObject responseObject = (JSONObject) this.DServer.responses.get(requestUUID);
